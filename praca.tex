...
\begin{figure}[!ht]
  \centering
  \includegraphics[width=14cm]{./Obrazy/usecase.png}
...
\begin{table}[H]
  ...
  \begin{center}
    \begin{tabular}{|l|l|}
      ...
      2            & \begin{tabular}[c]{@{}l@{}}Aplikacja powinna mieć możliwość ustawienia czas po jakim operacje \\ na protokole MQTT zostaną uznane za zakończone niepowodzeniem.\end{tabular} \\ \hline
      ...
      7            & \begin{tabular}[c]{@{}l@{}}Aplikacja musi obsługiwać przypadki, gdy otrzymana komenda do \\ wykonania przez BSP nie będzie poprawna.\end{tabular}                            \\ \hline
      8            & \begin{tabular}[c]{@{}l@{}}Aplikacja musi automatycznie potwierdzać lądowania urządzenie, jeżeli\\ jest to wymagane.\end{tabular}                                            \\ \hline
      9            & \begin{tabular}[c]{@{}l@{}}Aplikacja powinna mieć możliwość ustawiania interwału czasowego \\ w jakim zostaną przesłane dane dotyczące statusu statku.\end{tabular}          \\ \hline
      10           & \begin{tabular}[c]{@{}l@{}}Aplikacja powinna przesyłać obrazy w najwyższej dostępnej \\ rozdzielczości.\end{tabular}          \\ \hline
      10           & \begin{tabular}[c]{@{}l@{}}System musi być odporny na przerwy w dostępnie do sieci.                                \end{tabular}                                                         \\ \hline
...
Poniżej przedstawiono diagram architektury wysokopoziomowej całego systemu (Rys. 19.). Produktem widocznym dla użytkownika końcowego będzie storna webowa umożliwiająca kontrole i zarządzanie całym rojem dronów, podgląd ich danych, a także ich przetworzonych przez algorytmy sztucznej inteligencji. Najważniejszym takim algorytmem będzie rozpoznawanie obiektów umieszczonych na obrazie odebranym z BSP. W przypadku, w którym system miałby być wykorzystany w obszarze militarnym, rozpoznawaniem byłoby nastawione na ludzi i sprzęt wojskowy, tak aby operator roju, jak najszybciej uzyskiwał istotną na polu walki informacje.

Cała komunikacja odbywać się będzie za pomocą serwera MQTT, który tak jak każdy z elementów w tej architekturze w kolorze błękitnym może zostać zduplikowany. W ten sposób cała architektura jest odporna na zakłócenia komunikacyjne i uszkodzenia fizyczne sprzętu.

Celem tej pracy jest skonstruowanie oprogramowania umożliwiającego kontrole BSP i przesyłanie z niego danych za pomocą serwera MQTT (obszar oznaczony na rysunku kolorem pomarańczowym).


\begin{figure}[!ht]
  \centering
  \includegraphics[width=10cm]{./Obrazy/highlevel.png}
...
Przy projektowaniu abstrakcji komend, które można wydawać systemowi BSP, można wyróżnić dwa podejścia. Pierwsze z nich, które jest zaprezentowana poniżej (Rys. 21.), polega na przedstawianiu każdej z nich w postaci osobnej klasy z przyrostkiem \textit{Command}. Dziedziczą one po klasie \textit{Command}, która posiada abstrakcyjna metodę \textit{exec}. Celem tej metody jest zrealizowanie polecenia tożsamego z daną komendą, za pomocą obiektów dostarczonych przez DTO \textit{AircraftControllers}.
...
\subsubsection{Diagram klas ograniczony do serwisów dziłających w ramach komunikacji MQTT}
...
\subsection{Obsługa MQTT}
 W tej sekcji zaperzenotwano klasy związane z obsługą komunikacji za pomocą MQTT. Na poziomie najbliższym komponentowi \textit{FlightController} (Rys. 20.) ważne są wyróżnienia klasy \textit{MqttService} (Kod. 6.) i jej klasa wewnętrzna \textit{Topic} (Kod 5.). 
...
Ostatnim przeprowadzonym testem było przesłanie ręcznie na podany Topic związany z użytym dronem DJI pojedynczych treści. W tym celu skorzystano na komputerze osobistym z linuxem z pakietu \textit{mosquitto}. Następnie sprawdzono również otrzymywane dane, przy subskrybcji odpowiedniego Topic-a. Przeprowadzone działanie uświadczyło w przekonaniu, że aplikacja działa poprawnie.

\begin{figure}[!ht]
  \centering
  \includegraphics[width=15cm]{./Obrazy/exece.png}
  \caption{Zrzut ekranu prezentujące przesyłanie komend i odbieranie statusu staku powietrznego.}
  \source{Badania własne}
  \end{figure}
