...
Komunikacja dronów w ramach systemu będzie odbywała się za pomocą serwera MQTT. Będzie on odbierał dane ze statku i nim sterował za pomocą określonych komend. W ramach urządzenia mobilnego zostaną wyróżnione trzy komponenty:
  \begin{itemize}
  \item \textbf{Mqtt Services} to serwisy, które będą realizowały komunikacje z brokerem MQTT. Ich projekt i implementacja dożyć będzie do tego, aby API wystawione do komponentu \textit{FlightController} wymagało jak najmniej działania.
  ...
  \item \textbf{FlightController} będzie korzystał z dwóch wcześniej wymienionych komponentów, w nim będzie zawarta logika działania kontrolera. Praktycznie będzie stanowił jako punkt centralny w systemie z punktu widzenia pojedynczego drona.
  \end{itemize}
...
\begin{figure}[!htp]
  \centering
  \includegraphics[width=17cm]{./Obrazy/commands.png}
...
Przy określaniu 
  ...
  W centrum powyższego diagramu można wyróżnić 15. klas z przyrostekiem \textit{Command}. Będą one realizowały wykonywanie poleceń na BSP. Dziedziczą one po klasie abstrakcyjnej \textit{Command}, która posiada abstrakcyjną metode \textit{exec}, to w jej ciele będzie odbywało się realizowanie wybranych poleceń, za pomocą obiektu   diagram przedstawia komendy dostępne w ramach systemu. Klasa \textit

  W przypadku, w którym definicja komend miała by się ograniczać do definiowania w obiekcie typu enum, w \textit{CommandHandler} należało by obsłużyć każdy przypadek osobno, za pomocą struktury \textit{when}. Dodatkowo każda jej realizacja musiałaby być zdefiniowana w tej klasie, co zmniejszałoby czytelność całości. Z każdą kolejną komendą klasa ta rozrastałaby się do coraz większych rozmiarów. Takie podejście stanowiłoby naruszenie zasad SOLID \textit{Open/closed principle} i \textit{Single responsibility principle}. Zasady te zostały przedstawione przez autorytet programistów, Roberta C. Martina i są uznawane za dobre praktyki programowania obiektowego.\cite{solid}
\newpage

\begin{figure}[!htp]
  \centering
  \includegraphics[width=9cm]{./Obrazy/FlightStatus.png}
  \caption{Diagram klas klasy służącej do przechowywania stanu BSP}
  \source{Własne}
  \end{figure}
Diagram klas przedstawia \textit{FlightStatus}, która będzie przechowywała dane dotyczące statusu BSP w danej chwili czasu. Posiada ona tylko metodę \textit{toJson}. Zwraca ona stan obiektu w formacie JSON, które będzie przesyłany za pomocą serwisu MQTT. Klasy, które nie realizują żadnej logiki, a służą do przechowywania danych, określa się jako DTO (Data Transfer Object).
\newpage

\begin{figure}[!htp]
  \centering
  \includegraphics[width=16cm]{./Obrazy/services.png}
  \caption{Diagram klas klasy ograniczony do serwisów dziłających w ramach systemu}
  \source{Własne}
  \end{figure}
TODO
\newpage
...
\begin{figure}[!htp]
  ...
  \caption{Diagram klas klasy ograniczony do klas działających na DJI SDK}
  \source{Własne}
  \end{figure}
TODO
\newpage
...
\begin{figure}[!htp]
  ...
  \caption{DJI Mini 2}
  \source{https://laptrinhx.com/dji-mini-2-review-same-compact-size-more-confidence-flying-3163139258/}
  \end{figure}
...
\begin{figure}[!htp]
  ...
  \source{https://www.gohero.pl/userdata/public/gfx/6191/Kontroler-do-DJI-Mavic-Air-2.jpg}
  \end{figure}
...
\subsection{Klasa komend i jej wykonywanie}

\begin{lstlisting}[language=Java, caption=Klasa: Command]
...
\begin{lstlisting}[language=Java, caption=Klasa: StartMotorsCommand]
...
\begin{lstlisting}[language=Java, caption=Klasa: CommandFactory]
...
W ramach systemu utworzono klasę abstrakcyjną \textit{Command}. Reprezentuje ona komendy, które mogą zostać wysłane do wykonania przez BSP. Jej abstrakcyjna metoda \textit{exec}, odpowiada za zrealizowanie tej komendy na dronie. Operacje te są wykonywane za pomocą obiektu \textit{AircraftControllers}, który jest praktycznie wraperem na obiekty z SDK DJI, za pomocą których można wykonywać operacje np. startu. \textit{StartMotorsCommand} jest przykładem jej rozszerzenia, realizuje ona całą operację włącznie silników w BSP.

Odpowiedzialnością \textit{CommandFactory} jest generowanie instancji klas dziedziczących po \textit{Command}, na podstawie dostarczonego obiektu tekstowego w formacie JSON. Realizuje to metoda \textit{from}, która obsługuje przypadki, w których wartość tekstowa nie może zostać poprawnie rozpoznana, wtedy zwracany jest obiekt \textit{UnrecognizedCommand}.

Takie podejście architektoniczne sprawia, że realizacja komendy w \textit{CommandHandler} ogranicza się do 3 linii kodu. Dodanie kolejnej komendy do systemu ogranicza się do zdefiniowania klasy dziedziczącej po \textit{Command} i dodanie jednego przypadku w strukturze \textit{when} w metodzie \textit{createCommand} z \textit{CommandFactory}.
...
Klasa \textit{pl.edu.wat.droman.data.service.MqttService} zarządza Topic-ami MQTT. Dostarcza ona łatwy w obsłudze abstrakt (klasa \textit{pl.edu.wat.droman.data.service.MqttService.Topic}), a jego wygenerowanie odbywa sie na podstawie podanej ścieżki dla transmisji Mqtt. Można na nim wykonywa wykonywać opcje publikowania i subskrybowania wiadomości. Ponieważ zwraca ona obiekty za pomocą klasy \textit{androidx.lifecycle.LiveData} operacje te są wykonywane z kontekstem aplikacji. Dodatkowo w całym procesie wykorzystywany jest do tego serwer MQTT, którego parametry logowania i adresu są pobierane z pliku konfiguracyjnego gradle, który jest przechowywany na lokalnej maszynie. Takie zastosowanie pozwala na nie przechowywania kluczy wraz z repozytorium, cos jest istotne, jeżeli udostępnia się kod na zewnętrzne repozytorium lub uruchamia testy na środowiskach z różnym dostępem do serwera MQTT.

\begin{lstlisting}[language=Java, caption=Inicjalizowanie wartosci w klasie przed uruchominiem poszczególnych testów]
  @Before
    fun init() {
...
\begin{lstlisting}[language=Java, caption=Test publikowania danych za pomocą Mqtt]
  @Test
...
Jest to prosty przypadek testowy który sprawdza czy publikowanie wiadomości do serwera MQTT odbywa sie bez zwrócenia błedu. 

\begin{lstlisting}[language=Java, caption=Test pobierania danych za pomocą Mqtt]
  @Test
  fun getData() {
    //given
    val message = "message:" + UUID.randomUUID()
...
Klasa \textit{CommandFactory} odpowiada za generowanie obiektów klas komend, na podstawie obiektu, które są w późniejszym etapie wykonywane za pomocą klasy \textit{CommandHandler}. Klasy te są generowane na podstawie obiektu String w formacie JSON.

\begin{lstlisting}[language=Java, caption=Test tworzenia komendy \textit{UploadWaypointCommand} za pomocą \textit{CommandFactory}]
  @Test
...
Powyżej przykładowy test, w którym niestety nie można przetestować bezpośrednio wartości m.in. współrzędnych, ponieważ dostęp do nich jest chroniony. W aplikacji został wykorzystany framework Junit4, który nie umożliwia dostępu do obiektów chronionych, ale warto dodać, że inne, jak np. Spock, już takie rzeczy umożliwiają. Sam test służy do upewnienia się, że wartość tekstowa została zmapowana na poprawny obiekt.
...
Klasa \textit{TakeOffCommand}nie posiada w sobie żadnych parametrów, które wpływają na jej wykonanie, wiec problem, o którym była mowa powyżej nie występuje.

\begin{lstlisting}[language=Java, caption=Test tworzenia komendy \textit{UnrecognizedCommand} w przypadku nierozpoznania wartości za pomocą \textit{CommandFactory}]
...
Ten test sprawdza przypadek, w którym z jakiegoś powody nie będzie można utworzyć instancji klasy \textit{Command}, wtedy zostaje zwrócona specjalna komenda \textit{UnrecognizedCommand}. Taki sposób działa jest lepszy od zwracania wartości \textit{null}, co jest uznawane za złą praktykę.

\subsection{Testy autonomiczne}
W ramach systemy przygotowano dwa skrypty w Pythonie, który umożliwia przeprowadzanie testów automatycznych. 

\subsubsection{Publikowanie na topicu przeznaczonego do przesyłania komend} 
\begin{lstlisting}[language=Python, caption=Skryt przeznaczony do publikowania na nasłuchiwanym przez urządzenie latające topicu komend.]
...
Przedstawiony powyżej skrypt umożliwia w prosty sposób przesyłanie komend do urządzenia końcowego. Dodatkowo w ramach skryptu zostały udostępnione dwie sekwencje \textit{test} i \textit{test\_with\_take\_off}. Pierwsza z nich wykonuje trzy następujące po sobie komendy, które odpowiadają kolejno za:
\begin{itemize}
  \item uruchomienie silników;
  \item zrobienie zdjęcia;
  \item wyłącznie silników.
\end{itemize}
Druga z nich wykonuje dodatkowo wystartowanie, tj. uniesienie się na wysokość 1,4 metra i wylądowanie. Metoda \textit{test} w trakcie implementacji stanowiła szybko weryfikacje działania, nawet w zamkniętym pomieszczeniu.


\subsubsection{Subskrybowanie topicu przeznaczonego do przesyłania zdjęć}
...
\subsection{Coś o sprawdzeniu statusu statku powitrznego}
TODO
